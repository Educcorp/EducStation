...
export const formatDate = (dateString) => {
  ...
  /**
   * Decodifica todas las entidades HTML en un texto
  ...
  const decodeHTMLEntities = (text) => {
    ...
    // Mapa completo de entidades HTML más comunes
    const htmlEntities = {
      '&nbsp;': ' ',
      '&amp;': '&',
      '&lt;': '<',
    ...
    // Decodificar entidades numéricas hexadecimales (&#x1A3;)
  ...
  export const extractSummary = (content, maxLength = 150) => {
    ...
    // Primero, eliminar todos los scripts y estilos
    let cleanContent = content
      .replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, '')
      .replace(/<style\b[^<]*(?:(?!<\/style>)<[^<]*)*<\/style>/gi, '');
    
    // Convertir saltos de línea HTML a espacios
    ...
    // Decodificar entidades HTML múltiples veces si es necesario
    let previousText = '';
    let decodedText = cleanContent;
    let iterations = 0;
    const maxIterations = 5; // Prevenir bucle infinito
    
    while (previousText !== decodedText && iterations < maxIterations) {
