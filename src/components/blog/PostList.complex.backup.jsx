...
// Importar el hook personalizado, utilidades y componentesimport { usePosts } from './hooks/usePosts';import PostCard from './PostCard';import {   formatDate,   extractSummary,   renderImageHTML,  calculateReadingTime,  formatViews,  isBase64Image,  isHTMLImage} from './utils/postHelpers';
...
const PostList = ({ limit, categoryFilter, searchTerm, className, sortOrder = 'recientes' }) => {
  ...
  if (!displayPosts || displayPosts.length === 0) {
    ...
    return (    <div style={styles.container} className={className}>      {/* Grid de posts usando la misma estructura que CategoryPage */}      <div style={styles.postGrid}>        {displayPosts.map((post, index) => (          <div             className="post-card-animation"             key={post.ID_publicaciones}            style={{              "--animation-order": index,              background: "transparent"            }}          >            <PostCard post={post} />          </div>        ))}      </div>
